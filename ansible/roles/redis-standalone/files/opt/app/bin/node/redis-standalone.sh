...
configReplicaOf() {
  log "modify redis.conf of replicaof $1 started"
  # remove replicaof from redis.conf
  sed -i '/^replicaof/d' $RUNTIME_CONFIG_FILE
  if [ $# -gt 0 ]; then
    echo "replicaof $1 $REDIS_PORT" >> $RUNTIME_CONFIG_FILE
  fi
  log "modify redis.conf of replicaof $1 ended"
}

reConfigSentinel() {
  if [ $# -eq 0 ]; then return 0; fi
  log "re-config sentinel.conf started"
  # config sentinel monitor
  sed -i "s/^sentinel monitor.*/sentinel monitor $CLUSTER_ID $1 $REDIS_PORT $(getMonitorQuorum)/g" $RUNTIME_SENTINEL_FILE
  # remove auto generated configs
  sed -i '/# Generated by CONFIG REWRITE/q' $RUNTIME_SENTINEL_FILE
  log "re-config sentinel.conf ended" 
}
...
forceToMaster() {
  ...
  for nodeHost in $STABLE_REDIS_NODES; do
    ...
    if checkRedisStarted $nodeIp; then
      # wait 20s for all nodes checked and master started
      sleep 20
      log "other node are survived and ahead of me, set it as master."
      # re-config replicaof
      appctl stopSvc redis-server || :
      configReplicaOf $nodeIp
      appctl startSvc redis-server || :
      # re-config sentinel.conf
      reConfigSentinel $nodeIp
      # start sentinel
  ...
  # wait 10s for all nodes checked
  sleep 10
